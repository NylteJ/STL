...
#ifndef __EDG__ // FIXME: Likely an EDG bug. Reduced repro: https://godbolt.org/z/GGhTn7738
    ...
    void test_reserve() {
        hive_matrix(
            [&](hive_t& cont) {
                ...
                // > max_size()
                if (al_traits::max_size(al_1) < numeric_limits<size_ty>::max()) {
                    assert_throw<length_error>(
                        [&] { cont.reserve(static_cast<size_ty>(al_traits::max_size(al_1) + 1)); });
                }
            },
    ...
    void test_erase() {
        ...
        hive_matrix(
            [&](hive_t& cont) {
                ...
                if (cont.empty()) {
                    erase(cont, erase_proxy{ref(pred), raw_value_t{}});
                    assert(pred.cnt == 0);
                    assert(cont.empty());
                    return;
                }
                ...
                auto expect            = unwrap_to_vec(cont);
                erase(cont, erase_proxy{ref(pred), expect[0]});
        ...
        hive_matrix(
            [&](hive_t& cont) {
                if (cont.empty()) {
                    erase_if(cont, [](const T&) -> bool { abort(); });
                    assert(cont.empty());
                    return;
                }
...
constexpr auto maker = [](integral auto cnt, uniform_random_bit_generator auto& rnd_engine) {
    const auto single_maker = [&] {
        if constexpr (is_integral_v<Raw>) {
            ...
            return static_cast<Raw>(rnd_engine() % numeric_limits<Raw>::max());
        } else {
            ...
            Raw ret;
            ranges::generate(ret, [&] { return static_cast<value_t>(rnd_engine() % numeric_limits<value_t>::max()); });
            return ret;
        }
    };
...
void test_matrix() {
    ...
    // limits == {0, 0}
    {
        ...
        const auto matrix = [&](size_t cnt) {
            ...
            hive_t cont;
            if (cnt != 0) {
                assert_throw<length_error>([&] { cont.emplace(); });
            }
